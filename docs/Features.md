...
- [x] **时间轴日视图**：类似 iOS 日历 App 的日视图，按时间段规划每日任务。
    ...
    *   实时移动的"当前时间线"横穿任务块
- [x] **手势交互系统 (UIKit 增强版)**：
    *   使用 `UILongPressGestureRecognizer` 实现的工业级手势层，解决了 SwiftUI 原生手势与 `ScrollView` 的冲突。
    *   **长按即出块**：0.5s 长按瞬间获取精确坐标生成任务影子，无需手指位移。
    *   **无缝拖拽**：长按成功后手指不离屏即可直接调整时间位置。
    *   **吸附逻辑**：移动位置自动吸附 30 分钟刻度，调整时长吸附 15 分钟刻度。
    *   **防误触滚动**：快速滑动时手势自动失效，优先保证页面滚动顺滑。
